...
function PickerSheet({ children, Options = [], Title = "", Close = null }) {
  ...
  // giữ refs cho các component nested để không bị unmount
                      ...
                      {isByGroup && (
                        <div>
                          {Options.map((group, gIndex) => {
                            if (!componentRefs.current[gIndex])
                              componentRefs.current[gIndex] = {};
                            return (
                              <div
                                key={`group-${gIndex}`}
                              ...
                              >
                                ...
                                <div>
                                  {group.Options.map((child, cIndex) => {
                                    if (
                                      !componentRefs.current[gIndex][cIndex]
                                    ) {
                                      const Wrapper =
                                    ...
                                    return componentRefs.current[gIndex][
                                      cIndex
                                    ];
                                  })}
                                </div>
                      ...
                      {!isByGroup && (
                        <div>
                          {Options.map((item) => {
                            ...
                            // tạo stable ref key
                            if (!componentRefs.current[item.Title]) {
                              componentRefs.current[item.Title] = (
                                <Wrapper
                                  key={item.Title}
                                  open={open}
                                  close={close}
                            ...
                            return componentRefs.current[item.Title];
                          })}
                        </div>
                      )}
                    </div>
